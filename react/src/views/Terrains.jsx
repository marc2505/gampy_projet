...
export default function Terrains() {
    ...
    useEffect(()=>{
        ...
        .then(({data}) => {
            ...
            console.log(data)
            // if (data.message.includes('TOUS')) {
            //     setAucunChamp(true)
            //     setTerrains(data.terrains)
            // } else if (data.message.includes('jusqu\'à')) {
            //     setJusteDate(true)
            //     setTerrains(data.terrains)
            // } else if (data.terrains.length > 0) {
            //     setTerrains(data.terrains)
            // } else if (data.terrains.length == 0 && data.tousLesTerrains) {
    ...
    return (
        <div>
            ...
            {!loading && (
                <div className="container">
                    <h5>{msg} ({terrains.length > 0 ? terrains.length : tousLesTerrains.length} terrain{terrains.length > 0 ? terrains.length > 1 ? 's' : '' : tousLesTerrains.length > 1 ? 's' : '' }) : </h5>
                            ...
                            return (
                                <div key={cle}>
                                    <Link
                                        ...
                                        onClick={(e)=>{
                                            e.preventDefault()
                                            localStorage.setItem('valueStart',dateDebut)
                                            localStorage.setItem('valueEnd',dateFin)
